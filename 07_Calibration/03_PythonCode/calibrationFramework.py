...
def normalizePC6Consumption(year):
    cur = conn.cursor()
    cur.execute(
        """
...
def retrieveBaseLayer(year):
    cur = conn.cursor()
    cur.execute(
        """
            SELECT "buildingid", "postcodes", "address_num", "b_area", "volume", "datacomplete"
...
def main():
    ...
    cwd = os.getcwd()
    savePath = globalVariable.workingPath + '\simulationOutputs\\' + str(fileName) + str(simulationYear) +'.csv'

    computePC6archetype()
    normalizedLianderPC6consumption(simulationYear)
    normalizeSimuPC6consumption(simulationYear).to_csv(savePath)
