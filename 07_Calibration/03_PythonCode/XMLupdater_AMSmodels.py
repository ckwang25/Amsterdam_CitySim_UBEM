...
###### Initialize some variable settings which are defined globally
...
def queryBuildingInfo(buildingID, CaseID):
    ...
    cur.execute(
        """
        select tb1.*, tb2.*
        from "BuildingBaselayer" as tb1 join "ModelInputs" as tb2
...
def updateBuildingData(buildingRoot, buildingInfo, constructionIDs):
    ...
    Tmin = float(buildingInfo['p_Tmin'])
    Ninf = float(buildingInfo['p_Ninf'])
...
def updateAllinOneXML(caseN, treePath, simulationYear, saveToCloud = False):
    ...
    ###### Specify output XML file path here ######
    outputName = globalVariable.outputCityModelPrefix + str(caseN) + '.xml'
    if saveToCloud == True:
        outputPath = globalVariable.cloudFilePath + outputName
    else:
        outputPath = globalVariable.localFilePath + outputName
...
def main(N):
    ...
    # update N input combinations from input space
    for caseN in range(N):
        caseN += 1
        print "start updating XML case_" + str(caseN)+ " file..."

        simulationYear = globalVariable.simulationYear
        treePath = globalVariable.treePath
        xmlRoot = globalVariable.xmlRoot
        saveToCloud = globalVariable.saveToCloud

        # first time: collecting CitySim building volumes, slightly different to the building baselayer values
        if caseN == 1:
            retrieveModelGeometry.main(xmlRoot)
        updateAllinOneXML(caseN, treePath, simulationYear, saveToCloud)

    T2 = time.time()
    elapsed_time = T2 - T1
    print "processing time of updating XML file: ", elapsed_time
